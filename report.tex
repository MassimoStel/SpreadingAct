...
\textbf{Perché è necessario il sink?} Senza un meccanismo di dissipazione il sistema è chiuso. I grani entrano e non escono mai, superando rapidamente il limite massimo di configurazioni stabili tollerabili. Oltre quel limite, il sistema diverge (Fig.~\ref{fig:why}) e non si ferma mai. L'introduzione del sink permette invece di raggiungere uno stato stazionario stabile attorno al quale poter calcolare le statistiche in modo sensato.
...
\begin{itemize}
    ...
    \item \textbf{Saturazione Globale e Divergenza}: Rimuovendo il sink, la rete si riempie fino a raggiungere un punto in cui la configurazione non ammette più una stabilizzazione finita. Nel codice, questa instabilità viene intercettata dal parametro \texttt{max\_topplings} per evitare blocchi computazionali. Tuttavia, non si tratta di un'euristica arbitraria: come noto in letteratura (Dhar, 1990), per un sistema chiuso esiste un limite superiore matematico alla massa ospitabile (che con le nostre soglie di tolleranza corrisponde a $\sum_{v} \deg(v)$); superata tale soglia, il sistema \emph{deve} entrare in uno stato asintotico dal quale non può uscire in modo stabile. Il nodo-sink serve proprio a scaricare questa sovra-eccitazione. Guardando quanto tempo ci mette a bloccarsi (Fig.~\ref{fig:divergence}), emerge che stimolare un hub porta la rete al limite termodinamico più in fretta ($\rho_s = -0.43$).
