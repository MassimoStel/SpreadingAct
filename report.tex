...
descrivono un'attività che si propaga in cascate senza una scala caratteristica: non c'è una taglia "tipica", convivono eventi piccolissimi e valanghe enormi, e questo rende il sistema molto reattivo. In questo lavoro ho provato a vedere se anche la diffusione semantica nel lessico mentale si comporta così, proponendo un'alternativa ai classici modelli continui di spreading activation.
Per farlo, ho mappato la rete di associazioni libere \textbf{SWOW-EN} \cite{dedeyne2019} su un \textbf{modello Sandpile Abeliano}: un "grano di sabbia" rappresenta un'unità di attivazione che si posa su una parola. Ogni parola (nodo) può accumulare grani fino a un certo limite, dato dal suo numero di connessioni (il grado). Se supera quella soglia, il nodo diventa instabile e "crolla": cede un grano a ciascun vicino, e questo può innescare una reazione a catena, cioè una valanga.

\textbf{Come funziona} (Fig.~\ref{fig:anim}): per visualizzare la dinamica, in questo piccolo vicinato semantico ho iniettato i grani sempre sullo stesso nodo. L'attivazione si accumula finché il nodo supera la soglia e crolla, passando i grani ai vicini e propagando la valanga.
...
\caption{Finite-size scaling. Distribuzione di probabilità (log-binned PDF) per sottoreti di dimensioni diverse ($N=250, 500, 1000, 2500$, campionate tramite esplorazione BFS dalla componente gigante). Le curve si sovrappongono lungo quasi tutto il dominio, segno di una regione a legge di potenza condivisa; tuttavia, su queste taglie e con sottografi annidati, il cutoff non si sposta in modo chiaro al crescere di $N$ e non c'è un vero "data collapse". La FSS è quindi solo un indizio debole, non una prova: servirebbero sottografi indipendenti e una gamma di taglie più ampia.}
\label{fig:fss}
\end{figure}

\textbf{Perché è necessario il sink?} Senza un meccanismo di dissipazione il sistema è chiuso: i grani entrano e non escono mai, quindi la densità sale di continuo. Superata una certa densità il sistema non riesce più a stabilizzarsi e diverge (Fig.~\ref{fig:why}), con valanghe che non si fermano più. Non è un guasto, però: è la stessa transizione di fase di cui parlo nella sezione dopo. La divergenza è semplicemente l'ingresso nella fase attiva, il regime in cui l'attività si autoalimenta, e il caos comincia proprio dove l'analisi con il sink si ferma. Tra l'altro il sistema diverge prima ancora di riempirsi del tutto (prima di arrivare alla somma dei gradi, che è il massimo teorico). Il sink scarica i grani in eccesso e riporta tutto a uno stato stazionario, dove ha senso calcolare le statistiche.
...
\caption{Auto-organizzazione su un sottografo SWOW ($N=1000$, lo stesso della transizione di fase). L'\textbf{asse X} indica il tempo (iterazioni), l'\textbf{asse Y} la densità di grani $\rho(t)$. Partendo da zero, la densità converge spontaneamente verso un asintoto stazionario, illustrando il meccanismo dell'auto-organizzazione. L'asintoto si attesta però a $\rho\approx 16.4$, cioè circa metà del grado medio della rete ($\langle k\rangle\!\approx\!31$), e non coincide con la densità critica $\rho_c\!\approx\!2.1$ stimata dalla transizione di fase. Il valore stazionario dipende quindi dalla topologia (grado medio del sottografo) e non rappresenta una densità critica universale: l'identificazione SOC ``densità stazionaria $=\rho_c$'' qui non regge.}
\label{fig:iter}
\end{figure}

\section{Una conseguenza cognitiva: il fan effect}
Cercando un riscontro sul piano cognitivo (Fig.~\ref{fig:fan}), ho verificato se le parole-hub (quelle con molte connessioni) scatenino valanghe più grandi quando vengono attivate. Per ogni parola misuro quanto è grande in media la valanga che parte quando la attivo, cioè quando le aggiungo un grano. Testo tutte le parole a partire dalla stessa configurazione stazionaria di sfondo, così il confronto tra parole è equo (tutte partono dalle stesse condizioni). Il risultato è \textbf{negativo}: la correlazione di Spearman tra grado e taglia media della valanga è $\rho_s\!\approx\!-0.30$ ($p\!\approx\!10^{-11}$), e la correlazione parziale con la betweenness, controllando per il grado, è praticamente nulla ($\rho_{\text{partial}}\!\approx\!-0.03$).
Ha senso dal punto di vista fisico: nel modello la soglia di un nodo è proprio il suo grado, quindi gli hub sono i nodi più difficili da far crollare con una sola attivazione (sono robusti), mentre i nodi periferici a basso grado superano subito la soglia.
In altre parole, l'analogia con l'\textbf{effetto fan} \cite{anderson1974}, cioè l'idea che un concetto con molte associazioni disperda di più l'attivazione, da questo sandpile non viene fuori da sola: anzi, sono i nodi a grado medio-basso a scatenare le valanghe più grandi. Si potrebbe forzare un effetto fan apparente continuando a versare grani sullo stesso hub finché esplode, ma sarebbe solo una conseguenza dell'accumulo forzato, non una proprietà cognitiva del modello.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fan_effect_sandpile.png}
\caption{Effetto fan (sottografo $N=500$). \textbf{A sinistra}: taglia media della valanga per le parole-hub contro quelle periferiche; le due categorie sono di fatto indistinguibili (Mann--Whitney non significativo). \textbf{A destra}: grado del nodo (asse X) contro taglia media della valanga (asse Y). Non c'è alcuna crescita con il grado: la correlazione di Spearman è anzi leggermente negativa ($\rho_s\!\approx\!-0.30$). Le valanghe più grandi partono da nodi a grado medio-basso, non dagli hub, che avendo una soglia alta sono robusti alla singola attivazione.}
\label{fig:fan}
\end{figure}
...
\]
Quando crolla, il nodo cede un grano a ciascun vicino. In pratica perde tanti grani quanti sono i suoi vicini (uno per ciascuno) e nulla va perso (regola conservativa):
\[
...
"abeliana", cioè la configurazione finale non dipende dall'ordine con cui faccio crollare i nodi.

\textbf{Dissipazione.} I grani lasciano il sistema in due modi: tramite il "sink" (un nodo che assorbe i grani in eccesso, come il bordo aperto nel modello BTW) oppure con la dissipazione "di bulk", in cui ogni grano trasferito sparisce con probabilità $f$. Per l'analisi della criticità ho sempre tenuto $f=0$ (modello deterministico).
...
BFS per controllare la taglia). (2) Burn-in di $K=\sum_v\deg(v)$ passi per raggiungere lo
