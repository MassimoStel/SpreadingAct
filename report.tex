...
\caption{Finite-size scaling. Distribuzione di probabilità (log-binned PDF) per sottoreti di dimensioni diverse ($N=250, 500, 1000, 2500$, campionate tramite esplorazione BFS dalla componente gigante). Le curve si sovrappongono lungo quasi tutto il dominio, indicando una regione a legge di potenza condivisa; tuttavia, su queste taglie e con sottografi annidati, \emph{non} si osserva uno spostamento monotòno e netto del cutoff con $N$ né un vero \emph{data collapse}. La FSS va quindi considerata solo come evidenza debole, non conclusiva: una verifica rigorosa richiederebbe sottografi indipendenti e un intervallo di taglie più ampio.}
\label{fig:fss}
\end{figure}

\textbf{Perché è necessario il sink?} Senza un meccanismo di dissipazione il sistema è chiuso: i grani entrano e non escono mai, quindi la densità sale di continuo. Superata una certa densità il sistema non riesce più a stabilizzarsi e \emph{diverge} (Fig.~\ref{fig:why}), con valanghe che non si fermano. Vale la pena sottolinearlo: questa non è una patologia del modello, ma l'altra faccia della transizione di fase della sezione seguente. La divergenza coincide infatti con l'ingresso nella \emph{fase attiva}, lo stesso regime in cui --- a densità sufficiente --- l'attività si autosostiene; in pratica il regime caotico comincia proprio dove l'analisi con il sink si ferma. Questa soglia di divergenza è peraltro \emph{inferiore} al limite teorico massimo (la somma dei gradi, oltre cui nessuna configurazione stabile può esistere). Il sink, drenando i grani in eccesso, riporta il sistema in uno stato stazionario attorno al quale ha senso calcolare le statistiche.
...
\begin{itemize}
    \item \textbf{Topologia Non Orientata}: Per poter usare il modello Sandpile su questo grafo $G=(V,E)$, ho dovuto assumere che i legami fossero bidirezionali. Questa scelta sacrifica la reale asimmetria del lessico mentale (se la parola A fa venire in mente B, non è detto che valga il contrario con la stessa forza). Tuttavia, è un compromesso ragionevole: la versione classica e deterministica del sandpile (BTW) e gran parte degli strumenti analitici (tempo di rilassamento, taglia di valanga ben definita) sono sviluppati sul grafo non orientato. La proprietà \emph{abeliana} (il risultato finale di una valanga non dipende dall'ordine dei crolli) vale in realtà anche su grafi orientati purché ogni nodo possa dissipare verso un sink \cite{dhar1990}; la versione simmetrica è però la più semplice e standard da analizzare. L'asimmetria reale è recuperata separatamente con il modello stocastico di Manna (sezione finale).
    ...
    \item \textbf{Saturazione Globale e Divergenza}: Rimuovendo il sink, la rete si riempie fino a raggiungere un punto in cui la configurazione non ammette più una stabilizzazione finita. Nel codice, questa instabilità viene intercettata dal parametro \texttt{max\_topplings} per evitare blocchi computazionali. Tuttavia, non si tratta di un'euristica arbitraria: come noto in letteratura (Dhar, 1990), per un sistema chiuso la massa stabile è limitata superiormente dalla somma dei gradi $\sum_{v} \deg(v)$ (oltre cui nessuna configurazione stabile può esistere). In pratica, però, le dinamiche conservative smettono di stabilizzarsi già \emph{prima} di tale limite: quando la densità entra nella fase attiva, le valanghe non terminano più ed è lì che il sistema diverge (l'ho verificato alzando \texttt{max\_topplings} a $2\times10^6$, quindi non è un falso allarme del guard). Il nodo-sink serve proprio a scaricare questa sovra-eccitazione. Guardando quanto tempo ci mette a bloccarsi (Fig.~\ref{fig:divergence}), emerge che stimolare un hub porta la rete al limite termodinamico più in fretta ($\rho_s = -0.43$).
